...
토큰은 **관리자 비밀번호로 서명한 문자열**이다 — `만료시각.난수.HMAC-SHA256`.
서버가 검증만 하므로 **재시작해도 로그인이 풀리지 않는다**(유효기간 2주).
비밀번호를 바꾸면 기존 토큰이 한 번에 무효가 된다.
로그아웃은 프로세스 메모리의 폐기 목록(`_REVOKED`)에 담는다 — 재시작하면 비워진다.
...
- 401을 받으면 `requireLogin()`이 **열려 있던 과목·주차 모달을 먼저 감춘 뒤**
  로그인 창을 띄운다. 그러지 않으면 로그인 창이 모달 뒤에 가려져,
  저장을 눌러도 아무 일도 안 일어나는 것처럼 보인다(실제로 겪은 증상).
  다시 로그인하면 중단됐던 모달이 **입력값을 유지한 채** 되살아난다.
